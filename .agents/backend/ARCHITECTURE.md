...
JobHunter backend is a .NET 10 REST API organized as a 3-layer architecture. This document describes structure and dependency flow only. Coding conventions live in `CODING_RULES.md`.
...
- `Infrastructure/Persistence/`: EF Core `JobhunterContext`, migrations, and repository implementations.
...
This project references `JobHunter.Domain`.
...
This project references `JobHunter.Service`.
...
`-- ProjectReference: JobHunter.Service

JobHunter.Service.csproj
`-- ProjectReference: JobHunter.Domain
...
UseCase
    | uses
    |-- IRepository -> Repository -> JobhunterContext -> PostgreSQL
...
|-- AddCorsConfig()
|-- AddJwtConfig()
|-- AddDbContext<JobhunterContext>(UseNpgsql)
...
`AddApplicationServices()` is the main DI composition point for repositories and use cases inside `JobHunter.Service/Config/ServiceCollectionExtensions.cs`.

## Persistence

```text
JobHunter.Service/Infrastructure/Persistence/
|-- JobhunterContext.cs
|-- UserRepository.cs
|-- TokenRepository.cs
`-- Migrations/
...
The API configures `JobhunterContext` with PostgreSQL through `UseNpgsql(...)` in `Program.cs`. Migrations are stored beside the context under `Infrastructure/Persistence/Migrations`.
